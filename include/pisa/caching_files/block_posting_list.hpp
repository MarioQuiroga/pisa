...
#include "caching_files/pair_hash.hpp"
    ...
    struct block_posting_list {
        ...
        public:
            ...
            document_enumerator(uint8_t const* data, uint64_t universe,
                                size_t term_id = 0,
                                Cache<type_pair, Cache_data, pair_hash, Policy> * cache = 0)
        ...
        private:
            ...
            void PISA_NOINLINE decode_docs_block(uint64_t block)
            {
                ...
                // search in cache
                Cache_data * data = m_cache->get(type_pair(m_term_id, block));		
                if (data!= NULL){
                    m_docs_buf = data->block;	
                    if(data->m_freq_decoded){
                        m_freqs_buf = data->freqs;
                        uint8_t const* next_block = (const uint8_t*)data->next_ptr;
                        intrinsics::prefetch(next_block);
                        m_freqs_decoded = true;
                    }else{
                        m_freqs_block_data = (const uint8_t*)data->next_ptr;
